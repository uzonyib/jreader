...
public class FeedStatDaoImplTest extends AbstractDaoTest {
    ...
    public void find_ShouldReturnMatchingStat() {
        Optional<FeedStat> actual = sut.find(feed, 110L);

        assertEquals(actual.get(), stats.get(1));
    }

    @Test
    public void find_ShouldReturEempty_IfStatNotFound() {
        Optional<FeedStat> actual = sut.find(feed, 140L);
